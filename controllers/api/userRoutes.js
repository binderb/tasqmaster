...
router.post("/", async (req, res) => {
  try {
    if (!req.body.username || !req.body.email || !req.body.password) {
      res
        .status(403)
        .json({ message: "You must supply a username, email, and password!" });
      return;
    }
    ...
    if (err.name === "SequelizeUniqueConstraintError") {
      ...
      if (
        ...
        res
          .status(403)
          .json({ message: "You must provide a valid email address!" });
      } else if (
        err.message.includes(
...
router.post("/login", async (req, res) => {
  try {
    if (!req.body.username || !req.body.password) {
      res
        .status(403)
        .json({ message: "You must supply a username and password!" });
      return;
    }

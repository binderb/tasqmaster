...
router.post("/", withAuthAPI, async (req, res) => {
  try {
    if (!req.body.title) {
      ...
      res
        .status(403)
        .json({ message: "Your project description cannot be empty!" });
      return;
    } else if (!req.body.userList) {
      res
        .status(403)
        .json({ message: "You must give access to at least 1 user!" });
      return;
    }
  ...
  try {
    ...
    if (project.users.filter((e) => e.id == req.session.userID).length == 0) {
      res
        ...
        .json({
          message:
            "You are trying to delete a project over which you do not have ownership.",
        });
      return;
    }
